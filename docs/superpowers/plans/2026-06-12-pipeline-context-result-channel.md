...
| `lib/src/actions/pgyer_upload_action.dart` / `pgyer_upload_v2_action.dart` | 上传，产出下载链接 | 读 `get<File>(buildArtifact)`，`put(pgyerDownloadUrl)`，返回 `void` |
...
| `lib/src/actions/feishu_build_notify_action.dart` | 通知 | `downloadUrl`/`downloadUrls` → `downloadUrlKey`，读 `tryGet<String>` |
...
- [ ] **Step 2: 改 Feishu 测试 —— 用 downloadUrlKey**

`test/actions/feishu_build_notify_action_test.dart`：

- 第一个用例：把 `downloadUrl: 'https://example.com/dl',` 改为先 `context.put(ContextKeys.pgyerDownloadUrl, 'https://example.com/dl');`，action 构造改 `downloadUrlKey: ContextKeys.pgyerDownloadUrl,`。断言 `contains('https://example.com/dl')` 不变。
- 「formats message with multiple downloadUrls」用例：本设计移除多链接列表（spec Out of Scope）。删除该用例，新增一个等价的「无 key 时不含下载行」用例：

```dart
  test('omits download line when no downloadUrlKey provided', () async {
...
文件顶部 import 加 context_keys。

- [ ] **Step 3: 运行测试确认失败**

Run: `dart test test/actions/feishu_build_notify_action_test.dart`
Expected: 编译失败 —— `FeishuBuildNotifyAction` 还没有 `downloadUrlKey` 参数。

- [ ] **Step 4: Pgyer action 写 bag、返回 void**
...
```dart
      if (response['code'] == 0) {
        ...
        context.put(ContextKeys.pgyerDownloadUrl, url);
        return;
      }
```
...
```dart
    ...
    context.put(ContextKeys.pgyerDownloadUrl, downloadUrl);
```

（删除原 `return downloadUrl;`）

- [ ] **Step 5: FeishuBuildNotifyAction 改用 downloadUrlKey**
...
```dart
  ...
  /// Context key to read the download URL from; `null` means no link is shown.
  final String? downloadUrlKey;
  final ShellRunner _shellRunner;
```

- `_formatMessage` 内把读 url 的逻辑改为：

```dart
    final url =
        downloadUrlKey == null ? null : context.tryGet<String>(downloadUrlKey!);
...
`example/ci/pipelines/android_test_pipeline.dart` 第 42–49 行：
...
- **Spec 覆盖**：bag 形态（Task 1）、去泛型 R（Task 5）、迁移边界只动产出物（Task 2–4，config/infra 字段未触）、`buildName` 派生 getter（Task 2 Step 3）、`get` 抛错 / `tryGet` 返 null（Task 1）、集中常量 `ContextKeys`（Task 1）、通知读指定 key（Task 4）、多链接 Out of Scope（Task 4 Step 2 删除多链接用例）—— 均有对应任务。
- **类型一致**：全程 `get<T>` / `put` / `tryGet<T>`、`ContextKeys.{buildNumber,buildArtifact,pgyerDownloadUrl}`、`downloadUrlKey`，命名贯穿一致。
